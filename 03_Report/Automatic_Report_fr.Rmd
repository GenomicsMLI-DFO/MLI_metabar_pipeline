...
metabar.param %>% dplyr::filter(Locus %in% LOCUS) %>% dplyr::select(Locus, singleton.threshold, motus.correct, singleton.correct,	taxa.correct,	good.motus.contaminant.correct,	good.motus.contaminant.remove) %>% knitr::kable(caption = "Paramètres pour les MOTUs", col.names = str_replace_all(names(.), "[.]", " " ))
