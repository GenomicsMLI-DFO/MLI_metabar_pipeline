...
for(x in SUBGROUP){
  ...
  subgroup <- c(data.info %>% dplyr::filter(ID_subproject == x)  %>% dplyr::filter(Run == RUN) %>% pull(ID_subproject) %>% unique(),
                data.info %>% dplyr::filter(ID_subproject == x)  %>% dplyr::filter(Run == RUN) %>% pull(ID_project) %>% unique(),                                                         
                "ALL", "All", "all") %>% unique()
  
  id <- data.info %>% dplyr::filter(ID_subproject %in% subgroup) %>% dplyr::filter(Run == RUN)  %>% pull(ID_sample) %>% unique()
...
nreads.res <- cutadapt.res %>% left_join(dada2.summary %>% mutate( ID_sample = ID %>% str_remove(paste(paste0("_", LOCUS), collapse = "|"))) %>%
  ...
  left_join(data.info %>% select(ID_sample, Loci, ID_project, Sample_type, Run), 
                           by = c("ID_sample", "Loci"))  %>%                           
   mutate(Nreads = ifelse(is.na(Nreads), 0, Nreads),
                                    Nreads = ifelse((Sample_type %nin% c("ECH", "POOL") & Step == "ESVfinal"), NA, Nreads ),
                                    Nreads1 = Nreads + 1 ,
...
nmotus.res <- esv.motus.res %>% left_join(tag.summary %>% select(ID_sample = sample_id, ESVtagjump = nb_motus.tagjump, Loci)) %>% 
     left_join(metabar.summary %>% select(ID_sample = sample_id, ESVfinal = nb_motus_postmetabaR, Loci)) %>% 
               pivot_longer(c(Merge, ESVdada2, ESVtagjump, ESVfinal), names_to = "Step", values_to = "Nmotus") %>% 
                 ...
                 mutate(Nmotus = ifelse(is.na(Nmotus), 0, Nmotus),
                        Nmotus = ifelse((Sample_type %nin% c("ECH", "POOL") & Step == "ESVfinal"), NA, Nmotus ),
                        Nmotus1 = Nmotus + 1 ,
                        Step = factor(Step, levels = c("Raw", "Adapt", "Trim", "Merge", "ESVdada2", "ESVtagjump", "ESVfinal"))) 
...
ESV.table.control.long.int <- ESV.control.table %>% 
  dplyr::filter(ID_sample %in% SUBGROUP.ls[[projet]]) %>% 
                  pivot_longer(-ID_sample, names_to = "ID", values_to = "Nreads") %>% 
                  mutate(Loci = l) %>% 
                  left_join(data.info %>% select(ID_sample, ID_project, Sample_type, Region, Site) %>%  distinct(.keep_all = T )) %>% 
  dplyr::filter( Sample_type %nin% c("ECH", "POOL", "PPC", "MPC", "MPC_High", "MPC_Low")) %>% 
...
#   names(esv.int) <- c("ID_sample", seq.int)
#   esv.int <- esv.int %>% pivot_longer(-ID_sample, names_to = "SEQ", values_to = "Nreads") %>% 
#                          mutate(Loci = l)
#   
...
#ESV.table <- ESV.table %>% left_join(data.info %>% select(ID_sample, ID_project, Loci, Sample_type))

#ESV.table.summary <- ESV.table %>% group_by(ID_sample, Loci, Sample_type) %>% 
...
ESV.table.long.int <- ESV.table %>% pivot_longer(-ID_sample, names_to = "ID", values_to = "Nreads") %>% 
                  mutate(Loci = l) %>% 
                  left_join(data.info %>% select(ID_sample, ID_sample_ori, ID_project, Sample_type, Region, Site) %>%  distinct(.keep_all = T )) %>% 
...
graph.int <-   ESV.table.long %>% 
    mutate(Taxon= ifelse(is.na(Taxon), "Unassigned", Taxon),
         Groupe = ifelse(Loci %in% c("MiFishU", "MiMam", "12S160", "16Schord"), class, phylum)#,
  ...
  group_by(Loci, ID_sample, ID_sample_ori, Region, Site, Taxon, Groupe) %>% 
  summarise(Nreads = sum(Nreads)) %>% 
  ggplot(aes(fill = Nreads, x = ID_sample_ori, y = Taxon)) +
  labs(x= "", y = "") + 
  geom_bin2d(color = "darkgray")+
  scale_fill_distiller(trans = "log10",
                       palette = "Spectral",
...
readr::write_csv(ESV.table.long %>% group_by(Loci, ID_sample,  ID_sample_ori, Region, Site, Taxon, species, genus, family, order, class, phylum) %>% summarise(Nreads = sum(Nreads)), 
                 file = file.path("03_Report", paste0(projet, "_ESVtab_taxo_report.csv")))
